<div id="notifications">
    <a class="nav-link dropdown-toggle" href="#" id="alertsDropdown" role="button" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
        <span class="tc-notify-icon-wrap">
            <i class="fas fa-bell fa-fw"></i>
            @if(count(Auth::user()->unreadNotifications) > 0)
            <span class="badge badge-danger badge-counter count"
                data-count="{{ count(Auth::user()->unreadNotifications) > 5 ? 5 : count(Auth::user()->unreadNotifications) }}">
                {{ count(Auth::user()->unreadNotifications) > 5 ? '5+' : count(Auth::user()->unreadNotifications) }}
            </span>
            @endif
        </span>
      </a>
