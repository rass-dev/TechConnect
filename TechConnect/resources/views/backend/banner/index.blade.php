...
<div class="tc-page-card">
    <div class="tc-page-card-header">
        <div class="tc-page-card-title">
            ...
            <div>
                <h5>Banners List</h5>
                <p>Manage all site banners</p>
            </div>
        </div>
        <a href="{{ route('banner.create') }}" class="tc-btn-add">
        ...
        <div class="table-responsive">
            <table class="table tc-table" id="banner-dataTable" width="100%" cellspacing="0">
                ...
                <tbody>
                    ...
                    <tr>
                        <td class="tc-td-num">{{ $banner->id }}</td>
                        <td>
                            @if($banner->photo && file_exists(public_path($banner->photo)))
                                <img src="{{ asset($banner->photo) }}" class="tc-banner-thumb" alt="{{ $banner->title }}">
                            @else
                                <img src="{{ asset('backend/img/thumbnail-default.jpg') }}" class="tc-banner-thumb" alt="default">
                            @endif
                        </td>
                        <td><span class="tc-banner-title">{{ $banner->title }}</span></td>
                        <td>
                        ...
                        <td>
                            <div class="tc-action-btns">
                                ...
                                <form method="POST" action="{{ route('banner.destroy', [$banner->id]) }}" style="display:inline;">
                                    @csrf
                                    @method('delete')
                                    <button type="button"
...
<style>
    ...
    .tc-banner-title { font-weight: 600; color: #1F2340; }
